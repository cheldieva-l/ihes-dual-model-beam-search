...
The notebook first verifies the known 24-move path and the projection equation at all complementary path depths. The ordinary forward beam uses the validated primary MLP ranking; the reverse beam minimizes `mapped direct score - reverse score`. Both keep protection disabled and build complete retained frontiers. The notebook first performs the required blind exact set intersection. If it is empty, the labelled one-move extension scans every generated child of each full frontier against the opposite full frontier; those children are not reported as top-K. If the known audit trajectory drops from natural top-K, an optional separate diagnostic run enables last-slot protection and logs generation, natural retention, first drop, and protection. Protected frontiers are never used as blind-search evidence.
