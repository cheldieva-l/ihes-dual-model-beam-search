...
ranks candidates by `direct score - reverse score`. Its order is direction-aware:
for a reverse-primary candidate the mapped direct score is the first term. The IHES
implementation independently applies that mathematical orientation with the single
canonical MLP checkpoint selected by the run. No Tetraminx native source or weights
are copied.
